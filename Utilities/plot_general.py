...
def plot_pes_data(file_path, mode='2d', plot_type='x'):
    ...
    if mode == '2d':
        ...
        plt.xlabel('C-I Bond Length (Angstroms)')
        plt.ylabel('C-Cl  Bond Length (Angstroms)')
        plt.title('2D Contour Plot of Potential Energy Surface')
        plt.colorbar(label='Energy (kcal/mol)',shrink=1,fraction=0.05)
        plt.show()


    else:
        raise ValueError("Invalid mode. Choose '2d' or 'Contour'.")
